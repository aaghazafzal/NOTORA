  ...
  const { data: userBooks = [], isLoading: loadingBooks } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL || "http://localhost:9090"}/api/books?uploaderId=${user?.uid}`,
      );
  ...
  const startUpload = async () => {
    ...
    try {
      ...
      const eventSource = new EventSource(
        `${import.meta.env.VITE_API_URL || "http://localhost:9090"}/api/upload/progress?uploadId=${uploadId}`,
      );
      ...
      xhr.open(
        "POST",
        `${import.meta.env.VITE_API_URL || "http://localhost:9090"}/api/upload?uploadId=${uploadId}`,
        true,
      );
      ...
      <div className="mx-auto max-w-[1200px] px-4 py-6 sm:px-6 sm:py-10 space-y-8 animate-in fade-in duration-500">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <Button
            ...
            size="lg"
            className="rounded-full neon-glow shrink-0 shadow-lg"
          >
          ...
          {loadingBooks ? (
            ...
            <div className="flex flex-col items-center justify-center h-[300px] text-center max-w-sm mx-auto">
              ...
              <p className="text-muted-foreground text-sm mb-6">
                You haven't published any books to the community yet. Start sharing your knowledge
                today!
              </p>
              <Button
                onClick={() => setView("wizard")}
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-6 sm:px-6 sm:py-10 animate-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-display text-3xl font-black sm:text-4xl">Upload a book</h1>
          <p className="mt-1 text-sm text-muted-foreground">
        ...
        {step === 0 && (
          <div className="relative z-10 animate-in fade-in zoom-in-95 duration-300">
            ...
            <label className="flex cursor-pointer flex-col items-center justify-center rounded-3xl border-2 border-dashed border-primary/40 bg-background/50 p-12 text-center transition-all hover:bg-accent/30 hover:border-primary group">
              ...
              <span className="font-display text-xl font-semibold">Drop your EPUB or PDF here</span>
              <span className="mt-2 text-sm text-muted-foreground">Maximum file size: 300 MB</span>
              <input
                type="file"
                accept=".pdf,.epub"
                className="hidden"
            ...
            <div className="mt-10 flex justify-end">
              <Button
                onClick={() => setStep(1)}
                disabled={!bookFile}
                className="rounded-full neon-glow px-10 h-12 text-md font-semibold"
              >
                Continue to Cover
              </Button>
            </div>
        ...
        {step === 1 && (
          <div className="relative z-10 animate-in slide-in-from-right-8 duration-300">
            ...
            <label className="flex cursor-pointer flex-col items-center justify-center rounded-3xl border-2 border-dashed border-primary/40 bg-background/50 p-12 text-center transition-all hover:bg-accent/30 hover:border-primary group">
              ...
              <span className="font-display text-xl font-semibold">Drop high-res cover image</span>
            ...
            <div className="grid sm:grid-cols-2 gap-5">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  maxLength={100}
                  className="h-12 rounded-xl bg-background"
                  placeholder="classic, romance, 1920s"
                />
              </div>
            </div>
        ...
        {step === 3 && (
          <div className="relative z-10 animate-in slide-in-from-right-8 duration-300">
            ...
            <div className="mt-10 flex justify-between pt-4 border-t border-border/50">
              ...
              <Button
                onClick={startUpload}
                className="rounded-full neon-glow px-10 h-12 text-md font-bold"
              >
                Publish to Library
              </Button>
            </div>
