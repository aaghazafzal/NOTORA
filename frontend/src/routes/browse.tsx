  ...
  head: () => ({
    meta: [
      { title: "Browse the library — LumenPages" },
      {
        name: "description",
        content:
          "Search and filter thousands of e-books by genre, language, tag, or rating.",
      },
    ],
  }),
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant="outline" className="h-9 sm:h-10 rounded-xl border-border bg-card hover:bg-accent/50 transition-colors">
          <span className="font-medium text-xs sm:text-sm">{title}</span>
          {activeCount > 0 && (
            <Badge variant="secondary" className="ml-2 rounded-md px-1.5 py-0.5 text-[10px] bg-primary/20 text-primary hover:bg-primary/30">
              {activeCount}
            </Badge>
          )}
...
function BrowsePage() {
  ...
  const top3 = data?.pages[0]?.books?.slice(0, 3).map((b: any) => ({
    id: b._id,
    slug: b._id,
    title: b.title,
    authorName: b.author,
            ...
            <ul className="space-y-2">
              {ALL_LANGUAGES.map((l) => (
                <li key={l} className="flex items-center gap-2">
                  ...
                  />
                  <label htmlFor={`l-${l}`} className="text-sm cursor-pointer hover:text-primary transition-colors flex-1">
                    {l}
                  </label>
                </li>
              ))}
            ...
            <section className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {top3.map((book: any, i: number) => (
                  ...
                  >
                    {/* Rank Badge */}
                    <div className={`absolute -top-3 -left-3 w-8 h-8 rounded-full flex items-center justify-center font-black text-sm shadow-xl z-10 
                    ...
                    <div className="relative isolate aspect-[2/3] w-16 sm:w-20 rounded-lg overflow-hidden shadow-md shrink-0" style={coverStyle(book)}>
                      {book.coverUrl && (
                        <img src={book.coverUrl} alt={book.title} className="w-full h-full object-cover" />
                      )}
                    </div>
                    <div className="min-w-0">
                      <h3 className="font-bold font-display text-sm sm:text-base truncate group-hover:text-primary transition-colors">{book.title}</h3>
                      <p className="text-xs text-muted-foreground truncate">{book.authorName}</p>
                      <p className="text-[10px] uppercase font-semibold text-primary/70 tracking-wider mt-1">{book.genre}</p>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          {recentSearches.length > 0 && (
            <section className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Button variant="ghost" size="sm" onClick={() => { setRecentSearches([]); localStorage.removeItem("notora-recent-searches"); }} className="text-xs text-muted-foreground">
                  Clear All
                </Button>
              </div>
        ...
        <div className="space-y-6 pt-2 animate-in slide-in-from-bottom-4 duration-300">
          <div className="flex items-center justify-between">
            <h2 className="font-medium text-muted-foreground">
              {isLoading ? "Searching..." : `${data?.pages?.[0]?.totalCount ?? results.length} results found`}
            </h2>
          </div>
